...
{
    auto result = single_char_parser.parse(cstring_buffer("x"));
...
{
    auto result = single_char_parser.parse(cstring_buffer("z"));
    REQUIRE(!result.has_value());
}
...
{
    auto result = single_string_parser.parse(cstring_buffer("gogogo_ctpg"));
    REQUIRE(result.has_value());
    REQUIRE(result.value() == 1);
}
...
{
    auto result = single_string_parser.parse(cstring_buffer("what the heck"));
    REQUIRE(!result.has_value());
}
...
{
    auto result1 = single_regex_parser.parse(cstring_buffer("19"));
    REQUIRE(result1.has_value());
    REQUIRE(result1.value() == 1);

    auto result2 = single_regex_parser.parse(cstring_buffer("19123123"));
...
{
    auto result = single_regex_parser.parse(cstring_buffer("what the heck"));
    REQUIRE(!result.has_value());
}
