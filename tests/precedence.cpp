...
{
    auto result1 = test::p.parse(cstring_buffer("1+1"));
    REQUIRE(result1.has_value());
    REQUIRE(result1.value() == 2);

    auto result2 = test::p.parse(cstring_buffer("1-1"));
    REQUIRE(result2.has_value());
    REQUIRE(result2.value() == 0);

    auto result3 = test::p.parse(cstring_buffer("1*2"));
    REQUIRE(result3.has_value());
    REQUIRE(result3.value() == 2);

    auto result4 = test::p.parse(cstring_buffer("1#3"));
    REQUIRE(result4.has_value());
    REQUIRE(result4.value() == -2);

    auto result5 = test::p.parse(cstring_buffer("-2"));
    REQUIRE(result5.has_value());
    REQUIRE(result5.value() == -2);

    auto result6 = test::p.parse(cstring_buffer("(2)"));
...
{
    auto result1 = test::p.parse(cstring_buffer("2+2*2"));
    REQUIRE(result1.has_value());
    REQUIRE(result1.value() == 6);

    auto result2 = test::p.parse(cstring_buffer("(2+2)*2"));
    REQUIRE(result2.has_value());
    REQUIRE(result2.value() == 8);

    auto result3 = test::p.parse(cstring_buffer("2-2*2"));
    REQUIRE(result3.has_value());
    REQUIRE(result3.value() == -2);

    auto result4 = test::p.parse(cstring_buffer("(2-2)*2"));
    REQUIRE(result4.has_value());
    REQUIRE(result4.value() == 0);

    auto result5 = test::p.parse(cstring_buffer("-3+2"));
    REQUIRE(result5.has_value());
    REQUIRE(result5.value() == -1);

    auto result6 = test::p.parse(cstring_buffer("-(3+2)"));
    REQUIRE(result6.has_value());
    REQUIRE(result6.value() == -5);

    auto result7 = test::p.parse(cstring_buffer("-3#2"));
    REQUIRE(result7.has_value());
    REQUIRE(result7.value() == -5);

    auto result8 = test::p.parse(cstring_buffer("-(3#2)"));
    REQUIRE(result8.has_value());
    REQUIRE(result8.value() == -1);

    auto result9 = test::p.parse(cstring_buffer("3#2#1"));
